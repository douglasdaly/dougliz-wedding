...
def getenv_int(name: str, default: Optional[int] = None) -> int:
...
SQLALCHEMY_DATABASE_URI = f"{DB_ENGINE}://"
if DB_USER:
    SQLALCHEMY_DATABASE_URI += DB_USER
    if DB_PASSWORD:
        SQLALCHEMY_DATABASE_URI += f":{DB_PASSWORD}"
    if DB_HOST:
        SQLALCHEMY_DATABASE_URI += "@"
if DB_HOST:
    SQLALCHEMY_DATABASE_URI += DB_HOST
    if DB_PORT:
        SQLALCHEMY_DATABASE_URI += f":{DB_PORT}"
if DB_NAME:
    SQLALCHEMY_DATABASE_URI += f"/{DB_NAME}"
...
# Users
USERS_OPEN_REGISTRATION = getenv_bool("USERS_OPEN_REGISTRATION", True)
